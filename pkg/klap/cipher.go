...
func ivDerive(localHash []byte) ([]byte, int32) {
	localHash = append([]byte("iv"), localHash...)
	iv := sha256.Sum256(localHash)
...
func (c *KLAPCipher) Encrypt(data []byte) (payload []byte, seq int32, err error) {
	...
	padding := strings.Repeat(string(padSize), padSize)
...
func (c *KLAPCipher) Decrypt(data []byte) ([]byte, error) {
	...
	// decrypt data
	mode := cipher.NewCBCDecrypter(block, c.ivSeq(c.seq))
